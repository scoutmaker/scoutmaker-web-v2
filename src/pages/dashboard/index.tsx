...
const DashboardPage = () => {
  ...
  const { data: highestRatedReport, isLoading: highestRatedReportLoading } = useReports({
    limit: 2,
  ...
  return (
    <>
      ...
      <Container>
        <CreateCard linkTo="/reports/create" title={t('dashboard:CREATE_REPORT')} />
        <CreateCard linkTo="/notes/create" title={t('dashboard:CREATE_NOTE')} />
        <CountCard linkTo="/players" title={t('dashboard:PLAYERS_COUNT')} icon={<PlayersIcon />} count={players?.totalDocs} />
        <CountCard linkTo="/teams" title={t('dashboard:TEAMS_COUNT')} icon={<TeamsIcon />} count={teams?.totalDocs} />
        <CountCard linkTo="/reports" title={t('dashboard:REPORTS_COUNT')} icon={<ReportsIcon />} count={reports?.totalDocs} />
        <CountCard linkTo="/reports" title={t('dashboard:USER_REPORTS_COUNT')} icon={<ReportsIcon />} count={userReports?.totalDocs} />
        <CountCard linkTo="/notes" title={t('dashboard:NOTES_COUNT')} icon={<NotesIcon />} count={notes?.totalDocs} />
