...
import { useDeleteReportSkillAssessmentTemplate, useReportSkillAssessmentTemplates } from '@/modules/report-skill-assessment-templates/hooks'
...
import { ReportSkillAssessmentTemplatesFiltersDto, ReportSkillAssessmentTemplatesSortBy } from '@/modules/report-skill-assessment-templates/types'
...
const ReportSkillAssessmentTemplatesPage = ({ errorMessage, errorStatus }: TSsrRole) => {
  ...
  const [filters, setFilters] = useLocalStorage<ReportSkillAssessmentTemplatesFiltersDto>({
    key: 'report-skill-assessment-templates-filters',
    initialValue: initialFilters,
  })

  function handleSetFilters(newFilters: ReportSkillAssessmentTemplatesFiltersDto) {
    setFilters(newFilters)
    handleChangePage(null, 0)
  }

  const { data: reports, isLoading: dataLoading } = useReportSkillAssessmentTemplates({
    page: page + 1,
    limit: rowsPerPage,
  ...
  const { mutate: deleteReport, isLoading: deleteLoading } = useDeleteReportSkillAssessmentTemplate()
  ...
  return (
    <>
      {isLoading && <Loader />}
      <PageHeading title={t('report-skill-assessment-templates:INDEX_PAGE_TITLE')} />
      ...
      <ConfirmationModal
        ...
        message={t('report-skill-assessment-templates:DELETE_CONFIRM_QUESTION', {
          name: toDeleteData?.name,
        })}
        handleAccept={() => {
          if (toDeleteData)
            deleteReport(toDeleteData.id)
