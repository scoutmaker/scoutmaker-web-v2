...
export const ScoutProfileDetailsCard = ({ user }: IDetailsCard) => {
  const { t } = useTranslation()

  const { email, phone, region, city, club, profile } = user
  ...
  return (
    <Card sx={{ maxWidth: 700, margin: '0 auto' }}>
      <CardHeader
        ...
        title={getAuthorDisplayName(user)}
        titleTypographyProps={{ variant: 'h3' }}
      />
      <CardContent>
        <Grid container spacing={1}>
          <CardItemBasic
            title={t('scout-profiles:DATE_JOINED')}
            value={
          ...
          />
          <CardItemBasic title={t('RATING')} value={profile?.rating} />
          <CardItemBasic
